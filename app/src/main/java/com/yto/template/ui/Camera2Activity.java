...
public class Camera2Activity extends BaseActivity implements View.OnClickListener, ActivityCompat.OnRequestPermissionsResultCallback{
    ...
    protected void init(Bundle savedInstanceState) {
        ...
        adapter = new GridAdapter(paths,this);
        gridView.setAdapter(adapter);
//        mFile = new File(getExternalFilesDir(null), "m"+(int)System.currentTimeMillis()+"pic.jpg");
    }
    ...
    public void onResume() {
        ...
        if (mTextureView.isAvailable()) {
            System.out.println("onResume textureview width:"+mTextureView.getWidth()+" height:"+mTextureView.getHeight());
...
//                    .into(holder.image);
            ...
            new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
                    Glide.with(Camera2Activity.this).load(paths.get(position))
                            .centerCrop()
                            .placeholder(R.mipmap.timeline_img_default)
...
//        Activity activity = getActivity();
    ...
    private void takePicture() {
        if(paths.size()<3) {
            //每次拍摄时获取图片路径
            mFile = new File(getExternalFilesDir(null), "m" + (int) System.currentTimeMillis() + "pic.jpg");
            paths.add(mFile.getAbsolutePath());
            lockFocus();
        }else {
            showToast("最多只能拍三张");
        }
...
//            final Activity activity = getActivity();
            ...
            CameraCaptureSession.CaptureCallback CaptureCallback
                ...
                public void onCaptureCompleted(@NonNull CameraCaptureSession session,
                    ...
                    ///storage/emulated/0/Android/data/com.example.android.camera2basic/files/pic.jpg
...
//                    mHandler.sendEmptyMessageAtTime(100,50);
                }
            };
    ...
    private static class MyHandler extends Handler{
        ...
        private final WeakReference<Camera2Activity> mActivty;

        private MyHandler(Camera2Activity mActivty) {
            this.mActivty = new WeakReference<Camera2Activity>(mActivty);
        }

        @Override
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            Camera2Activity activity = mActivty.get();
            if (activity != null){
    ...
    private void showImage() {
        ...
        Glide.with(Camera2Activity.this).load(file).into(tempImg);
    }
